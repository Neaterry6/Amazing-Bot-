...
class MessageHandler {
  ...
  shouldProcessNoPrefix(text, isGroup, group) {
    ...
    return isGroup ? group?.settings?.noPrefixEnabled === true : config.privateNoPrefixEnabled === true;
  }
    ...
    try {
      ...
      if (response) {
        await sock.sendMessage(message.key.remoteJid, {
          text: response,
          contextInfo: { mentionedJid: isGroup ? [user.jid] : undefined },
        });
        return true;
      }
    } catch (error) {
      ...
      await sock.sendMessage(message.key.remoteJid, {
        text: '❌ Ilom’s AI hit a snag! Try again, please! 😎',
      }, { quoted: message });
    }
  ...
  async handleMentions(sock, message, text, isGroup) {
    ...
    const mentionedUsers = mentions.map(mention => mention.replace('@', '') + '@s.whatsapp.net');
    const from = message.key.remoteJid;
    const metadata = await sock.groupMetadata(from);
    const validMentions = mentionedUsers.filter(jid => metadata.participants.some(p => p.id === jid));

    if (validMentions.length > 0) {
      await updateGroup(from, { $inc: { mentionsCount: validMentions.length } });
    }
  }
  ...
  async handleIncomingMessage(sock, message) {
    try {
      ...
      // Prioritize active chat handlers (e.g., Ilom chat sessions)
      ...
      // Process commands (handles aliases like .chat, .ilom, .talk, .ai)
      const isCommand = await this.processCommand(sock, message, messageContent.text, user, group, isGroup);
      if (isCommand) return;

      // Handle auto-replies or chatbot if no command
      const isAutoReply = await this.handleAutoReply(sock, message, messageContent.text, user, isGroup);
  ...
  async handleMessageError(sock, message, error) {
    try {
      const from = message.key.remoteJid;
      const isOwner = config.ownerNumbers.includes(message.key.participant || from);
  ...
  async handleMemberJoin(sock, groupId, participant, group, metadata) {
    try {
      ...
      if (!user) {
        ...
        await updateUser(participant, { $addToSet: { joinedGroups: groupId } });
      }
      ...
      if (group.settings?.autoRoleEnabled) {
        ...
        await updateUser(participant, { [`groupRoles.${groupId}`]: autoRole });
      }
    } catch (error) {
  ...
  async handleMemberPromote(sock, groupId, participant, group, metadata) {
    try {
      ...
      await updateUser(participant, { [`groupRoles.${groupId}`]: 'admin' });
  ...
  async handleMemberDemote(sock, groupId, participant, group, metadata) {
    try {
      ...
      await updateUser(participant, { [`groupRoles.${groupId}`]: 'member' });
