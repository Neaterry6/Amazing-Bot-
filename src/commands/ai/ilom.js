...
function inferUserStyle(history = []) {
    const userTexts = history.filter(h => h.role === 'user').map(h => h.text).filter(Boolean).slice(-8);
    if (!userTexts.length) return 'Use clear, friendly English.';

    const joined = userTexts.join(' ');
    const shortCount = userTexts.filter(t => t.split(/\s+/).filter(Boolean).length <= 8).length;
    const emojiCount = (joined.match(/[\u{1F300}-\u{1FAFF}]/gu) || []).length;
    const slangWords = ['abeg', 'omo', 'dey', 'una', 'pls', 'bro', 'wahala', 'lol', 'lmao'];
    const slangHits = slangWords.filter(w => new RegExp(`\\b${w}\\b`, 'i').test(joined));

    const styleBits = [];
    if (shortCount >= Math.ceil(userTexts.length / 2)) styleBits.push('Prefer short replies.');
    if (emojiCount > 0) styleBits.push('Use light emojis naturally.');
    if (slangHits.length) styleBits.push(`Mirror mild slang when suitable: ${slangHits.join(', ')}.`);
    styleBits.push('Stay helpful and easy to understand.');
    return styleBits.join(' ');
}

async function extractDocumentText(buffer, documentMessage = {}) {
    const fileName = String(documentMessage.fileName || 'document').toLowerCase();
    const mime = String(documentMessage.mimetype || '').toLowerCase();
    ...
    if (
        ...
        /\.(txt|md|json|js|ts|csv|xml|html|env|yaml|yml)$/i.test(fileName)
    ) {
...
async function analyzeImage(buffer, prompt = 'Describe this image in detail.') {
    ...
    form.append('file', new Blob([buffer], { type: 'image/jpeg' }), 'ilom-image.jpg');
    const uploadRes = await axios.post('https://tmpfiles.org/api/v1/upload', form, { timeout: 45000 });
    const pageUrl = uploadRes?.data?.data?.url;
    if (!pageUrl) throw new Error('Could not upload image for Google Lens.');

    const directUrl = String(pageUrl).replace('https://tmpfiles.org/', 'https://tmpfiles.org/dl/');
    const lensUrl = `https://lens.google.com/uploadbyurl?url=${encodeURIComponent(directUrl)}`;
    const { data: html } = await axios.get(lensUrl, {
        headers: { 'user-agent': 'Mozilla/5.0' },
        timeout: 45000
    });

    const source = String(html || '');
    const titleMatch = source.match(/<title[^>]*>([^<]+)<\/title>/i)?.[1]?.trim();
    const bestGuess = source.match(/Best guess for this image[^<]*<\/span>\s*<span[^>]*>([^<]+)/i)?.[1]?.trim()
        || source.match(/"text":"([^"]{3,120})"/i)?.[1]?.trim();

    const lensSummary = [bestGuess, titleMatch].filter(Boolean).join(' | ');
    if (!lensSummary) {
        return 'Google Lens could not confidently identify this image. Try a clearer image.';
    }

    return askAI(`You are Ilom. User prompt: ${prompt}\nGoogle Lens hints: ${lensSummary}\nGive a concise visual analysis.`);
}

async function analyzeAudio(buffer, prompt = 'Transcribe this audio and summarize what is said.') {
    if (!ASSEMBLY_API_KEY) {
        throw new Error('AssemblyAI API key missing. Set ASSEMBLYAI_API_KEY.');
    }

    const headers = { authorization: ASSEMBLY_API_KEY };
    ...
    if (!audioUrl) throw new Error('Failed to upload audio to AssemblyAI.');
    ...
    if (!transcriptId) throw new Error('Failed to create AssemblyAI transcript job.');

    let status = transcriptRes?.data?.status;
    let text = transcriptRes?.data?.text || '';
    let summary = '';

    for (let i = 0; i < 40 && status !== 'completed' && status !== 'error'; i++) {
        await new Promise(resolve => setTimeout(resolve, 3000));
        const poll = await axios.get(`https://api.assemblyai.com/v2/transcript/${transcriptId}`, { headers, timeout: 30000 });
    ...
    if (status !== 'completed') {
        throw new Error('AssemblyAI transcription did not complete in time.');
    }

    const combined = [text?.trim(), summary?.trim() ? `\nSummary: ${summary.trim()}` : ''].filter(Boolean).join('\n');
    if (prompt && /summar/i.test(prompt) && text) return combined;
    return text || combined || 'I could not transcribe that audio.';
}
...
export default {
    ...
    async execute({ sock, message, args, from, sender, isGroup, isBotAdmin, isOwner, isSudo }) {
        ...
        if (/\b(open|read|extract|copy)\b.*\b(file|document|doc|pdf)\b|\b(send|show|paste)\b.*\b(content|text)\b/i.test(input)) {
            const quoted = extractQuotedMessage(message);
            const documentMessage = quoted?.documentMessage;
            if (!documentMessage) {
                return await sock.sendMessage(from, { text: '❌ Reply to a document/file with: ilom read this file' }, { quoted: message });
            }

            try {
                const docBuffer = await sock.downloadMediaMessage({ message: { documentMessage } });
                const docText = await extractDocumentText(docBuffer, documentMessage);
                if (!docText) {
                    return await sock.sendMessage(from, { text: '❌ I could not read this file type yet. Use txt/md/json/js/csv/xml/html/env/yml/pdf.' }, { quoted: message });
                }

                const wantsRaw = /\b(send|show|paste|copy)\b.*\b(full|raw|all|entire)\b|\bexact\b|\bverbatim\b/i.test(input);
                const output = wantsRaw ? docText : docText.slice(0, 7000);
                const suffix = wantsRaw || docText.length <= 7000 ? '' : '\n\n…(truncated, ask: "ilom send full content")';
                await appendHistory({ from, sender, role: 'user', text: `[Shared document: ${documentMessage.fileName || 'file'}]` });
                await appendHistory({ from, sender, role: 'assistant', text: output.slice(0, 600) });
                return await sock.sendMessage(from, { text: `📄 *File Content*\n\n${output}${suffix}` }, { quoted: message });
            } catch (err) {
                return await sock.sendMessage(from, { text: `❌ Failed to open/read this file.\n${err.message}` }, { quoted: message });
            }
        }
        ...
        const chain = async (replyText, replyMessage) => {
            ...
            const chainStyle = inferUserStyle(chainHistory);
            const follow = await askAI(`Continue as Ilom.\nReply style guide: ${chainStyle}\n\nConversation history:\n${buildHistoryPrompt(chainHistory)}\n\nUser: ${sub}`);
